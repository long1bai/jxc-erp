...
import com.jxc.erp.dto.PhotoDtos.ConfirmItem;
 ...
 * 2026-08-04 重构：确认入库改走 PurchaseCreationService（checkApproval=false，不走审批、直接加库存，行为不变）。
 */
@Service
public class PhotoService {
    ...
    public PhotoService(DashScopeClient dash, SupplierMapper supplierMapper,
                        MaterialMapper materialMapper, PurchaseCreationService purchaseCreationService,
    ...
    public Map<String, Object> confirm(Long supplierId, String poDate, String remark,
        ...
        List<ConfirmItem> clean = new ArrayList<>();
        for (var it : items) {
        ...
        // 组装明细行 → 走公共创建服务（checkApproval=false：拍照确认不走审批，直接加库存）
        ...
        for (var it : clean) {
            ...
            lines.add(new PurchaseCreationService.Line(it.materialId(), mName, mSpec, mUnit, "",
                    it.quantity(), it.unitPrice()));
        }
        var draft = new PurchaseCreationService.Draft(null, supplierId, null, "",
                poDate, "", remark, warehouseId, lines);
        return purchaseCreationService.create(draft, false);
    }
