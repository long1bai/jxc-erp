...
public interface TradeMapper {
    ...
    @Select("<script>SELECT po.id, po.po_no, po.po_date, po.supplier_id, COALESCE(s.name,'') AS supplier_name, " +
            "po.project_id, COALESCE(po.project_name,'') AS project_name, " +
            "po.ext_json, po.total_quantity, po.total_amount, po.remark, po.created_at " +
    ...
    /** 采购单列表导出（非分页，含 ext_json 动态字段，LIMIT 5000） */
    @Select("<script>SELECT po.po_no, po.po_date, COALESCE(s.name,'') AS supplier_name, " +
            "COALESCE(po.handler,'') AS handler, COALESCE(po.project_name,'') AS project_name, " +
            "po.ext_json, po.total_quantity, po.total_amount, COALESCE(po.remark,'') AS remark " +
            "FROM purchase_orders po LEFT JOIN suppliers s ON s.id = po.supplier_id " +
            "<where>po.deleted = 0 <if test='kw != null and kw != \"\"'>AND " +
            "(po.po_no LIKE CONCAT('%',#{kw},'%') OR COALESCE(s.name,'') LIKE CONCAT('%',#{kw},'%'))" +
            "</if></where> ORDER BY po.id DESC LIMIT 5000</script>")
    List<Map<String, Object>> purchaseExport(@Param("kw") String kw);

    /** 采购单动态字段（ext_json）落库：新建单据后单独更新 */
    @Update("UPDATE purchase_orders SET ext_json = #{extJson} WHERE id = #{id} AND deleted = 0")
    int purchaseUpdateExt(@Param("id") Long id, @Param("extJson") String extJson);
    ...
    @Insert("INSERT INTO purchase_orders (po_no, supplier_id, project_id, project_name, po_date, handler, total_quantity, total_amount, remark) " +
            "VALUES (#{poNo}, #{supplierId}, #{projectId}, #{projectName}, #{poDate}, #{handler}, #{totalQty}, #{totalAmt}, #{remark})")
    int purchaseInsert(@Param("poNo") String poNo, @Param("supplierId") Long supplierId,
                       @Param("projectId") Long projectId, @Param("projectName") String projectName,
    ...
    @Insert("INSERT INTO purchase_items (po_id, material_id, material_name, spec, unit, assembly_system, quantity, unit_price, amount, sort_order) " +
            "VALUES (#{poId}, #{materialId}, #{materialName}, #{spec}, #{unit}, #{assemblySystem}, #{quantity}, #{unitPrice}, #{amount}, #{sort})")
    int purchaseItemInsert(@Param("poId") Long poId, @Param("materialId") Long materialId,
                           ...
                           @Param("unit") String unit, @Param("assemblySystem") String assemblySystem,
    ...
    /** po_no 是否存在（含逻辑删除残留——po_no UNIQUE 键不分 deleted，导入幂等判重必须用这个，否则重传撞唯一键） */
    @Select("SELECT id FROM purchase_orders WHERE po_no = #{poNo} LIMIT 1")
    Long purchaseExists(@Param("poNo") String poNo);
