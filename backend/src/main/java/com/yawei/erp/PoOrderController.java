...
/** 独立采购订单（订单 → 分批入库 → 执行跟踪）
 *  与入库单解耦：po_orders 管"订什么/收了多少/是否完成"，
 *  实际入库走现有 purchase_orders（按单入库时生成入库单 + 加库存 + 更新订单执行状态）。
 *  2026-08-01 新增（此前采购单=下单+入库一体，392 张历史单保持原语义不受影响）。 */
@RestController
...
public class PoOrderController {
    ...
    public PoOrderController(TradeMapper mapper, SysMapper sys, SequenceUtil seq) {
        this.mapper = mapper;
        this.sys = sys;
        this.seq = seq;
    }
    ...
    public Map<String, Object> create(@RequestBody CreateReq req) {
        ...
        String no = seq.nextDaily(Constants.SEQ_PO_ORDER);
        ...
        return ApiResponse.ok(Map.of("id", id, "poOrderNo", no));
    }
    ...
    public Map<String, Object> detail(@PathVariable Long id) {
        ...
        return ApiResponse.ok(Map.of("main", rows.get(0), "items", mapper.poOrderItems(id)));
    }
    ...
    public Map<String, Object> receive(@PathVariable Long id, @RequestBody List<ReceiveReq> items) {
        ...
        if ("done".equals(status)) {
            return ApiResponse.fail("该订单已全部入库");
        }
        ...
        for (var it : items) {
            ...
            if (it.quantity().compareTo(rem) > 0) {
                return ApiResponse.fail("物料 #" + it.materialId() + " 本次入库超过剩余未收数量（剩余 " + rem + "）");
            }
        }
        ...
        mapper.purchaseInsert(poNo, (Long) order.get("supplier_id"), today(), String.valueOf(order.get("handler") == null ? "" : order.get("handler")),
        ...
        return ApiResponse.ok(Map.of("poId", poId, "poNo", poNo, "orderStatus", allDone ? "done" : "partial"));
    }
    ...
    public Map<String, Object> delete(@PathVariable Long id) {
        ...
        if (linked != null && linked > 0) {
            return ApiResponse.fail("该订单已有入库单，不能删除（如需作废请删除对应入库单）");
        }
