...
### 整体布局（2026-08-01 简化版，Layout.vue）
- **三栏 → 两栏**：顶部快捷功能条已删除（6 项与左侧菜单完全重复，用户反馈冗余）；保留 左侧主菜单 + 右侧实时统计面板
- 左侧：后端下发的菜单树（/api/menus 按角色过滤，8 组全量平铺）
- 右侧「实时统计」面板（仅 PC，212px）：按当前模块显示指标卡片（进销存→待出货/部分出货/未发货；报表→库存预警/本月销售/采购；财务→待收/待付/本月收款；生产/报工→本月送货/新订单），数据复用 /api/dashboard/data（60 秒缓存），点击数字跳转对应页面
- 顶部：标题 + 用户名 + 退出（手机端另有底部导航 5 项）
- 模块关联：路由变化自动选中所属菜单组（selectedModule），驱动右侧面板和菜单标题
