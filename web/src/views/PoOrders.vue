<template>
  <el-card shadow="never">
    ...
    <div class="search-bar">
      <el-input v-model="keyword" placeholder="搜索单号/供应商" clearable style="width: 220px"
                @keyup.enter="load(1)" @clear="load(1)">
        <template #prefix><el-icon><Search /></el-icon></template>
      </el-input>
      <el-select v-model="statusFilter" placeholder="状态" clearable style="width: 130px" @change="load(1)">
    ...
    <el-table :data="items" v-loading="loading" size="small" stripe @row-click="openDetail" v-if="!isMobile">
      <el-table-column prop="po_order_no" label="订单号" min-width="160" />
      <el-table-column prop="order_date" label="日期" width="105" />
      ...
      <el-table-column prop="total_quantity" label="总数量" width="90" align="right" />
    ...
    <div v-else class="m-cards">
      <div v-for="row in items" :key="row.id" class="m-card" @click="openDetail(row)">
        <div class="m-card-head">
          <span class="m-name">{{ row.po_order_no }}</span>
          <el-tag :type="statusType(row.status)" size="small">{{ statusText(row.status) }}</el-tag>
        </div>
        <div class="m-card-body">
          ...
          <div class="m-row"><span>总金额</span><b>￥{{ (row.total_amount ?? 0).toLocaleString() }}</b></div>
        </div>
      </div>
    ...
    <!-- 新建采购订单弹窗 -->
    <el-dialog v-model="createVisible" title="新建采购订单" width="820px" top="4vh">
      <el-form ref="createFormRef" :rules="createRules" label-width="80px" size="small">
        <el-row :gutter="10">
          ...
          <el-col :span="6">
            <el-form-item label="经手人"><el-input v-model="createForm.handler" placeholder="经办人" /></el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="备注"><el-input v-model="createForm.remark" placeholder="订单备注（可选）" /></el-form-item>
    ...
    <el-dialog v-model="pickerVisible" title="选择物料" width="640px">
      ...
      <el-table :data="pickerItems" size="small" max-height="380" highlight-current-row
        ...
        <el-table-column prop="purchase_price" label="采购价" width="80" align="right" />
    ...
    <!-- 订单详情弹窗（含分批入库） -->
    <el-dialog v-model="detailVisible" :title="'采购订单 ' + (detail.main?.po_order_no || '')" width="820px" top="4vh">
      <el-descriptions :column="isMobile ? 1 : 3" size="small" border class="mb">
        ...
        <el-descriptions-item label="经手人">{{ detail.main?.handler || '—' }}</el-descriptions-item>
      ...
      <el-table v-if="!isMobile" :data="detail.items" size="small" border>
        ...
        <el-table-column prop="quantity" label="订购数量" width="95" align="right" />
        <el-table-column prop="received_quantity" label="已收" width="90" align="right" />
      ...
      <div v-else class="m-detail-items">
        <div v-for="it in detail.items" :key="it.id" class="m-detail-item">
          ...
          <div class="m-di-line"><span>订购</span><b>{{ it.quantity }} {{ it.unit }}</b></div>
          <div class="m-di-line"><span>已收</span><b>{{ it.received_quantity || 0 }}</b></div>
          <div class="m-di-line"><span>剩余</span><b>{{ remaining(it) }}</b></div>
        </div>
        <div v-if="!detail.items.length" class="m-empty">无明细</div>
      </div>

      <!-- 分批入库区 -->
      <div v-if="detail.main?.status !== 'done'" class="receive-box">
        <div class="receive-title">本次入库数量</div>
        <el-table :data="receiveItems" size="small" border>
          ...
          <el-table-column label="本次入库" width="120">
            <template #default="{ row }">
              <el-input v-model="row.quantity" type="number" min="0" :max="row.remaining" placeholder="0" />
            </template>
          </el-table-column>
        </el-table>
        <div class="receive-actions">
          <el-button type="primary" size="small" :loading="receiving" @click="doReceive">确认入库</el-button>
        </div>
      </div>
    </el-dialog>
  </el-card>
...
const createForm = reactive({ supplierId: null, orderDate: '', handler: '', remark: '', items: [] })
...
function statusText(s) {
  return { pending: '待入库', partial: '部分入库', done: '已完成' }[s] || s || '—'
}
function statusType(s) {
  return { pending: 'warning', partial: 'primary', done: 'success' }[s] || 'info'
}
function remaining(row) {
  const q = Number(row.quantity) || 0
  const r = Number(row.received_quantity) || 0
...
async function load(p) {
  ...
  loading.value = true
  try {
    const res = await api.poOrders({ keyword: keyword.value, status: statusFilter.value, page: page.value, size: size.value })
...
function openCreate() {
  Object.assign(createForm, { supplierId: null, orderDate: today(), handler: '', remark: '', items: [newRow()] })
...
async function remove(row) {
  try {
    await ElMessageBox.confirm(`确定删除采购订单「${row.po_order_no}」？`, '删除确认', { type: 'warning' })
  } catch {
    return
  }
  try {
    await api.deletePoOrder(row.id)
...
async function openDetail(row) {
  try {
    const res = await api.poOrderDetail(row.id)
...
async function doReceive() {
  const valid = receiveItems.value
  ...
  for (const it of receiveItems.value) {
    if (Number(it.quantity) > it.remaining) {
      ElMessage.warning(`「${it.materialName}」本次入库超过剩余数量`)
      return
    }
  }
  receiving.value = true
  try {
    const res = await api.receivePoOrder(detail.value.main.id, valid)
    ElMessage.success(`入库成功（订单状态：${statusText(res.data.orderStatus)}）`)
...
onMounted(() => { load(1); loadSuppliers() })
</script>
