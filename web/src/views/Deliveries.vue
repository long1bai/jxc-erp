<template>
  <el-card shadow="never">
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" :title="'送货单 ' + (detail.main?.dn_no || '')" width="820px">
...
async function load(p) {
  ...
  loading.value = true
  try {
    const res = await api.deliveries({ keyword: keyword.value, page: page.value, size: size.value })
