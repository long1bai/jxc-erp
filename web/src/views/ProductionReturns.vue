<template>
  <el-card shadow="never">
    ...
    <el-alert type="info" :closable="false" show-icon style="margin-bottom: 10px"
              title="车间剩余/报工多扣的材料退回仓库（生成 in 流水，自动冲回材料库存）。" />

    <div class="search-bar">
      <el-input v-model="keyword" placeholder="搜索退料单号" clearable size="small" style="width: 200px"
    ...
    <el-table :data="items" size="small" stripe v-loading="loading" max-height="480" v-if="!isMobile">
      <el-table-column prop="prt_no" label="退料单号" min-width="160" />
      <el-table-column prop="return_date" label="日期" width="100" sortable />
      <el-table-column prop="total_quantity" label="退料数量" width="110" align="right" sortable />
      ...
      <template #empty><el-empty description="暂无退料单" :image-size="60" /></template>
    </el-table>
    <!-- 卡片（手机） -->
    <div v-else class="m-cards">
      <div v-for="row in items" :key="row.id" class="m-card">
        <div class="m-card-head">
          <span class="m-name">{{ row.prt_no }}</span>
        </div>
        <div class="m-card-body">
          <div class="m-row"><span>退料单号</span><b>{{ row.prt_no }}</b></div>
          <div class="m-row"><span>日期</span><b>{{ row.return_date }}</b></div>
          <div class="m-row"><span>退料数量</span><b>{{ row.total_quantity }}</b></div>
    ...
    <el-dialog v-model="createVisible" title="新建退料单" width="820px" destroy-on-close>
      <el-form label-width="80px" size="small">
        <el-form-item label="日期">
          <el-date-picker v-model="form.returnDate" type="date" value-format="YYYY-MM-DD" style="width: 160px" />
        </el-form-item>
        ...
        <el-form-item>
          <el-button size="small" @click="form.items.push({ materialId: null, quantity: 1 })"><el-icon><Plus /></el-icon> 添加物料</el-button>
        </el-form-item>
    ...
    <el-dialog v-model="viewVisible" title="退料单明细" width="640px">
      <el-descriptions :column="3" size="small" border style="margin-bottom: 10px">
        <el-descriptions-item label="退料单号">{{ viewMain.prt_no }}</el-descriptions-item>
      ...
      <el-table :data="viewItems" size="small" border>
        <el-table-column prop="material_name" label="物料" min-width="180" />
...
const viewMain = ref({})
const viewItems = ref([])
const form = reactive({ returnDate: '', remark: '', items: [] })
...
async function load(p) {
  ...
  loading.value = true
  try {
    const r = await request.get('/production-returns', { params: { keyword: keyword.value, page: page.value, size: size.value } })
...
async function save() {
  if (!form.items.length || form.items.some(i => !i.materialId)) return alert('请填写退料明细')
  saving.value = true
  try {
    const r = await request.post('/production-returns', form)
...
async function view(row) {
  const r = await request.get(`/production-returns/${row.id}`)
...
async function remove(row) {
  if (!confirm(`确认删除退料单 ${row.prt_no}？库存将自动回退。`)) return
  await request.delete(`/production-returns/${row.id}`)
  load(page.value)
}
