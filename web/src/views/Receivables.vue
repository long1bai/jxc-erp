    ...
    <el-tabs v-model="tab">
      ...
      <el-tab-pane label="📊 汇总" name="summary">
        <el-table :data="summary" size="small" stripe max-height="520">
          ...
          <el-table-column prop="total_amount" label="应收总额" width="130" align="right" sortable>
            <template #default="{ row }">{{ fmt(row.total_amount) }}</template>
          </el-table-column>
          <el-table-column prop="settled_amount" label="已收款" width="130" align="right" sortable>
            <template #default="{ row }">{{ fmt(row.settled_amount) }}</template>
          </el-table-column>
          <el-table-column prop="balance" label="未收款" width="130" align="right" sortable>
        ...
        <div class="sum-bar">合计：应收 <b>{{ fmt(sumOf('total_amount')) }}</b> ｜ 已收 <b>{{ fmt(sumOf('settled_amount')) }}</b> ｜ 未收 <b class="text-danger">{{ fmt(sumOf('balance')) }}</b></div>
      </el-tab-pane>

      <!-- ============ 单据（金蝶式） ============ -->
      <el-tab-pane label="🧾 单据" name="docs">
        <div class="search-bar">
          <el-input v-model="filter.kw" placeholder="搜索单号/客户" clearable size="small" style="width: 200px"
          ...
          <el-button type="warning" size="small" :disabled="!selectedDoc" @click="openSettle">
            <el-icon><Link /></el-icon> 收款核销
          </el-button>
        </div>

        <el-table :data="docs" size="small" stripe highlight-current-row @current-change="selectDoc"
                  v-loading="docLoading" max-height="420">
          <el-table-column prop="doc_no" label="送货单号" min-width="175" />
          <el-table-column prop="doc_date" label="日期" width="100" />
          <el-table-column prop="party_name" label="客户" min-width="150" show-overflow-tooltip />
          <el-table-column prop="amount" label="应收款" width="110" align="right" sortable>
            <template #default="{ row }">{{ fmt(row.amount) }}</template>
          </el-table-column>
          <el-table-column prop="settled_amount" label="已收款" width="100" align="right" sortable>
            <template #default="{ row }">{{ fmt(row.settled_amount) }}</template>
          </el-table-column>
          ...
          <el-table-column label="开票状态" width="110">
            <template #default="{ row }">
              <el-tag :type="row.uninvoiced_amount <= 0 ? 'success' : row.invoiced_amount > 0 ? 'warning' : 'danger'" size="small">
                {{ row.uninvoiced_amount <= 0 ? '已开票' : row.invoiced_amount > 0 ? '部分开票' : '未开票' }}
              </el-tag>
            </template>
          </el-table-column>
        ...
        <div class="settle-box" v-if="selectedDoc">
          ...
          <el-table :data="settlements" size="small" max-height="160">
            <el-table-column prop="voucher_no" label="收款单号" min-width="160" />
      ...
      <el-tab-pane label="📅 分月汇总" name="monthly">
        <el-table :data="monthly" size="small" stripe max-height="520">
          ...
          <el-table-column prop="sales" label="销售金额" align="right" sortable>
            <template #default="{ row }">{{ fmt(row.sales) }}</template>
          </el-table-column>
          <el-table-column prop="received" label="当月收款" align="right" sortable>
            <template #default="{ row }">{{ fmt(row.received) }}</template>
          </el-table-column>
          <el-table-column label="净额" align="right">
            <template #default="{ row }"><b :class="row.sales - row.received > 0 ? 'text-danger' : ''">{{ fmt(row.sales - row.received) }}</b></template>
          </el-table-column>
          <template #empty><el-empty description="暂无数据" :image-size="60" /></template>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="⏳ 账龄分析" name="aging">
        <el-table :data="aging" size="small" stripe max-height="520">
          <el-table-column prop="party_name" label="客户" min-width="170" show-overflow-tooltip />
          ...
          <el-table-column prop="total_balance" label="未收合计" width="120" align="right">
            <template #default="{ row }"><b>{{ fmt(row.total_balance) }}</b></template>
          </el-table-column>
          <template #empty><el-empty description="暂无未结清应收" :image-size="60" /></template>
        </el-table>
      </el-tab-pane>
    </el-tabs>

    <!-- 收款核销弹窗 -->
    <el-dialog v-model="settleVisible" title="收款核销" width="640px">
      <el-alert type="info" :closable="false" class="mb">
        单据「{{ selectedDoc?.doc_no }}」未结余额 <b>{{ fmt(selectedDoc?.balance) }}</b>，选择该客户的收款单进行核销。
      </el-alert>
      <el-form ref="settleRef" :rules="settleRules" label-width="90px" size="small">
        <el-form-item label="收款单" required prop="voucherId">
          <el-select v-model="settleForm.voucherId" filterable placeholder="选择收款单" style="width: 100%">
            <el-option v-for="v in candidateVouchers" :key="v.id" :value="v.id"
                       :label="`${v.rv_no}（${fmt(v.amount)}，剩余 ${fmt(v.amount - (v.settled_amount || 0))}）`" />
          </el-select>
        </el-form-item>
...
async function loadSummary() {
  const res = await api.receivableSummary()
  summary.value = res.data.items
}
async function loadMonthly() {
  const res = await api.receivableMonthly()
...
async function loadDocs(p) {
  ...
  try {
    const params = { type: 'sales', page: docPage.value, size: docSize.value, keyword: filter.kw }
...
async function selectDoc(row) {
  ...
  await loadSettlements(row)
}
async function loadSettlements(row) {
  if (!row) return
  const res = await api.settlements({ refType: 'delivery', refId: row.id })
...
async function openSettle() {
  ...
  settleForm.remark = ''
  try {
    const res = await api.vouchers('receipts', { keyword: selectedDoc.value.party_name, page: 1, size: 100 })
...
async function doSettle() {
  ...
  if (!settleForm.voucherId || !settleForm.amount) {
    ElMessage.warning('请选择收款单并填写金额')
    return
  }
  settling.value = true
  try {
    await api.settle({
      refType: 'delivery', refId: selectedDoc.value.id,
      voucherId: settleForm.voucherId, amount: settleForm.amount, remark: settleForm.remark,
    })
    ElMessage.success('核销成功')
    settleVisible.value = false
    await Promise.all([loadDocs(), loadSettlements(selectedDoc.value), loadSummary()])
  } catch (e) {
...
async function loadAging() {
  const res = await api.aging({ type: 'sales' })
